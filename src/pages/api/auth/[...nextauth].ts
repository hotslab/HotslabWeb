...
export default NextAuth({
    ...
    callbacks: {
        session: async ({ session, token }: CallbackSession) => {
            ...
            const user = await prisma.user.findUnique({ where: { id: parseInt(token.sub) }, include: { role: true, profile: { select: { imageUrl: true } } } })
            if (user) {
                ...
                session.user.image = user.profile?.imageUrl
            }
            return session
        }
    },
