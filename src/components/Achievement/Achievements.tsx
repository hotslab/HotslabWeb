...
export default function Achievements({ achievements, profile, close }: props) {
    ...
    async function deleteItem() {
        if (selectedAchievement) {
            ...
            await fetch(
                `${process.env.NEXT_PUBLIC_HOST}/achievement/${selectedAchievement.id}`,
                {
                    method: "DELETE",
                    headers: {
        ...
        } else eventBus.dispatch("openErrorModal", "Achievment selected for deletion is missing")
    }
    function closeEdit() {
            ...
            }
            {
                ...
                <Modal>
                    <h2 className="text-gray-900 text-xl">
                        <span>Delete </span>
                        <span className="text-error">{selectedAchievement?.name}</span>
                        ?
                    </h2>
